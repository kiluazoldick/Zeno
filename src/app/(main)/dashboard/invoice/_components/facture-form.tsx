...
import { CalendarIcon, CreditCard, FileText, GripVertical, Hash, Plus, Settings, Trash2, User } from "lucide-react";
...
import {
  defaultFactureValues,
  type FactureFormValues,
  type FactureLineItem,
  factureClients,
  factureTaxOptions,
  getLineAmount,
} from "./facture-data";
...
interface FactureFormProps {
  onValuesChange?: (values: FactureFormValues) => void;
}

export function FactureForm({ onValuesChange }: FactureFormProps) {
  const form = useForm<FactureFormValues>({
    defaultValues: defaultFactureValues,
  });
  const values = useWatch({ control: form.control }) as FactureFormValues;
      ...
      >
        <Tabs defaultValue="facture">
          <TabsList className="w-full">
            <TabsTrigger value="facture" className="gap-2">
...
function FactureDetails() {
  const { control, register } = useFormContext<FactureFormValues>();
            ...
            <Field className="gap-1">
              ...
              <Select
                ...
                onValueChange={(clientId) => {
                  const nextClient = factureClients.find((item) => item.id === clientId);
...
function FactureItems() {
  ...
  return (
    <section className="flex flex-col gap-4">
      <div className="flex items-center justify-between gap-3">
        <h2 className="font-medium tracking-tight">Lignes de la facture</h2>
...
function SortableFactureItemRow({
  id,
  index,
  item,
  register,
          ...
          render={({ field }) => (
            <Field className="gap-1">
              ...
              <Select value={field.value} onValueChange={field.onChange}>
                ...
                <SelectContent>
                  <SelectGroup>
                    {factureTaxOptions.map((taxOption) => (
...
function FactureNotes() {
  const { register } = useFormContext<FactureFormValues>();
  ...
  return (
    <section className="flex flex-col gap-3">
      ...
      <div className="grid gap-4">
        ...
        <Field className="gap-1">
          <FieldLabel className="text-xs">Conditions de paiement</FieldLabel>
          <Textarea
            className="min-h-20 resize-none text-sm"
            {...register("conditions")}
